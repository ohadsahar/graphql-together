...
var possiblyAddFilterInput = function possiblyAddFilterInput(astNode, typeMap, resolvers, config) {
          ...
          if (relationTypeDirective) {
            ...
            if (fromType === toType) {
              ...
              if (typeMap[reflexiveFilterName] === undefined) {
                ...
                typeMap[relationFilterName] = (0, _graphql.parse)('input ' + relationFilterName + ' {' + relationTypeFilters.join('') + '}');
              }
            } else {
              if (typeMap[relationFilterName] === undefined) {
                ...
                typeMap[relationFilterName] = (0, _graphql.parse)('input ' + relationFilterName + ' {' + _relationTypeFilters.join('') + '}');
              }
            }
          }
          if (!isList) {
            ...
            if (_relationFilterType) {
              filterFields.push('\n                ' + fieldName + ': ' + _relationFilterType + '\n                ' + fieldName + '_not: ' + _relationFilterType + '\n                ' + fieldName + '_in: [' + _relationFilterType + '!]\n                ' + fieldName + '_not_in: [' + _relationFilterType + '!]\n                ' + fieldName + '_some: ' + _relationFilterType + '\n                ' + fieldName + '_none: ' + _relationFilterType + '\n                ' + fieldName + '_single: ' + _relationFilterType + '\n                ' + fieldName + '_every: ' + _relationFilterType + '\n              ');
            }
          }
        }
      });
  ...
  if (typeMap[filterType] === undefined && filterFields.length) {
    typeMap[filterType] = (0, _graphql.parse)('input ' + filterType + ' {' + filterFields.join('') + '}');
  }
  return typeMap;
};
  ...
  fields.forEach(function (field) {
    ...
    if (fieldType && fieldIsNotIgnored(astNode, field, resolvers) && shouldAugmentType(config, 'query', fieldTypeName)) {
      ...
      if ((0, _utils.isNodeType)(fieldType)) {
        ...
        var filterTypeName = '_' + astNode.name.value + fieldTypeName + 'Filter';
        if (fromType === toType) {
